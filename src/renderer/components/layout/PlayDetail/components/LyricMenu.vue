<template>
  <teleport to="#root">
    <div ref="dom_menu" :class="$style.container" :style="menuStyles" :aria-hidden="!modelValue">
      ...
      <div :class="$style.group">
        <div :class="$style.subGroup">
          <button :class="[$style.btn, { [$style.active]: appSetting['playDetail.style.fontWeight'] }]" :aria-label="$t('lyric_menu__font_weight')" @click="toggleFontWeight">
            {{ $t('lyric_menu__font_weight') }}
          </button>
        </div>
      </div>
      <div :class="$style.group">
...
import { appSetting, setPlayDetailLyricFont, setPlayDetailLyricAlign, setPlayDetailLyricFontWeight } from '@renderer/store/setting'
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const toggleFontWeight = () => {
      setPlayDetailLyricFontWeight(!appSetting['playDetail.style.fontWeight'])
    }
